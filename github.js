...
GitHub.listWorkflowsForRepo = async function listWorkflowsForRepo(
  repoName,
  repoOwner
) {
  try {
...
GitHub.getUsage = async function getUsage(
  repoOwner,
  repoName,
  workflowId,
  run_id
...
GitHub.getMostRecentRuns = async function getMostRecentRuns(
  repoOwner,
  repoName,
  workflowId
) {
...
GitHub.refreshWorkflow = async function refreshWorkflow(
  repoOwner,
  repoName,
  workflowId
) {
    ...
    for (const repo of repos) {
      ...
      if (workflows.length > 0) {
        for (const workflow of workflows) {
          ...
          const runs = await GitHub.getMostRecentRuns(
            repo.owner.login,
            repo.name,
            workflow.id
          );
